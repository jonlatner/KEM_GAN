...
write.csv(df_comparison, paste0(tables,"datasynthesizer_optimize_utility_variables.csv"), row.names=FALSE)


# Graph ----

df_comparison <- read.csv(paste0(tables,"datasynthesizer_optimize_utility_variables.csv"))
...
df_graph <- ggplot(df_comparison, aes(x = variables, y = specks, fill = parents)) +
  ...
  facet_wrap( ~ data, labeller = labeller(.rows = label_both), scales = "free_y", nrow = 3) +
...
ggsave(plot = df_graph, paste0(graphs,"datasynthesizer_optimize_utility_variables.pdf"), height = 4, width = 6)
