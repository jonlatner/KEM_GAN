...
df_graph <- ggplot(tidy_model_data_list, aes(y = term, x = estimate, 
  ...
  geom_point() +
  geom_errorbar() +
  facet_wrap(~dv, labeller = labeller(.cols = label_both), nrow = 1, scales = "free_x") +
...
df_graph <- ggplot(tidy_model_data_list, aes(y = term, x = estimate, 
  ...
  geom_point() +
  geom_errorbar() +
  facet_grid(copies~dv, labeller = labeller(.cols = label_both, .rows = label_both), scales = "free_x") +
  labs(y = "Variables", x = "Estimate coefficeint with 95% CI") +
  theme_bw() +
  geom_vline(xintercept = 0, linetype = "solid", color = "red") +
  theme(panel.grid.minor = element_blank(), 
