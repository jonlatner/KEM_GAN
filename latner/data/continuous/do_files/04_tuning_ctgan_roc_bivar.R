...
for (col_name in continuous_var_names) {
  bins <- seq(min(df_ods_binned[[col_name]]), max(df_ods_binned[[col_name]]), length.out = 20) # 21 points to get 20 bins
      ...
      for (b in batch) {
        ...
        for (f in frequency) {
          for (j in 1:c) {
            ...
            for (col_name in continuous_var_names) {
              bins <- seq(min(sds[[col_name]]), max(sds[[col_name]]), length.out = 20) # 21 points to get 20 bins
