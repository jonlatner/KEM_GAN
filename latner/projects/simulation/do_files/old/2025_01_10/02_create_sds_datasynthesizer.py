        ...
        for p in parents:
            for e in epsilon:
                for j in range(m):    
                    ...
                    df_sds['copies'] = m
                    df_sds['number'] = j
                    df_synthetic = pd.concat([df_synthetic, df_sds], ignore_index=True)
...
df_synthetic = df_synthetic.astype(str)
full_path = os.path.join(synthetic_data, 'datasynthesizer_synthetic_data.csv')
