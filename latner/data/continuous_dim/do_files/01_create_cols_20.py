...
for r in rows:
    ...
    # Bimodal distribution variables with mean approximately 100
    bimodal_vars = {}
    for i in range(1, 6):
        bimodal_data = np.concatenate([np.random.poisson(90, r // 2), np.random.poisson(110, r // 2)])
        np.random.shuffle(bimodal_data)  # Shuffle the data to mix the two distributions
        bimodal_vars[f"bimodal_var{i}"] = bimodal_data
        
    # Combine all variables into a single dictionary
    all_vars = {**normal_vars, **left_skewed_vars, **right_skewed_vars, **bimodal_vars}
    ...
    filename = f"ods_rows_{r}_cols_20.csv"
