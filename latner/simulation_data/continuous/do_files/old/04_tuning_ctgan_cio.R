    ...
    for (b in batch) {
      for (f in frequency) {
        for (m in copies) {
          # Loop through each continuous variables as dependent variable
          for (dep_var in vars_continuous) {
            sds <- read.csv(paste0(synthetic_data,"sds_ctgan_tuning_e_",e,"_d_",d,"_b_",b,"_f_",f,"_m_",m,"_n_1.csv"))
...
data_for_clustering_long <- data_for_clustering %>%
  pivot_longer(cols = !c(epochs, discriminator, batch, frequency, copies,n), names_to = "names", values_to = "value")
...
ggsave(plot = df_graph, paste0(graphs,"graph_ctgan_cio_raw.pdf"), height = 4, width = 6)

## Linear model

lm_model_1 <- lm(ci_overlap ~ epochs + discriminator + batch + frequency + copies, data = data_for_clustering)
lm_model_2 <- lm(std_diff ~ epochs + discriminator + batch + frequency + copies, data = data_for_clustering)
...
df_graph <- ggplot(tidy_model_data, aes(y = term, x = estimate, 
  ...
  geom_point() +
  geom_errorbar() +
  facet_wrap(~dep_var, scales = "free_x") +
...
ggsave(plot = df_graph, paste0(graphs,"graph_ctgan_cio.pdf"), height = 4, width = 6)
...
for (d in dv) {
  ...
  lm_model <- lm(paste0(d, " ~ epochs + discriminator + batch + frequency"), data = subset(data_for_clustering, copies == c))
...
ggsave(plot = df_graph, paste0(graphs,"graph_ctgan_cio_facet.pdf"), height = 9, width = 6)
...
# df_output_cio %>% filter(epochs==900, discriminator==10, batch==1000, frequency == "False", copies == 5 & dep_var == "var_1_B") %>% select(mean_std_coef_diff,mean_ci_overlap_noNeg)
