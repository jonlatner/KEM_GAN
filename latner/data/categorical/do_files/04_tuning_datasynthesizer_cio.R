  ...
  for (dv in dep_var) { 
    for (k in parents) {
      for (e in privacy) {
        ...
        # model 1
        ## ods
        glm_model_ods <- glm(as.formula(paste0(dv, " ~ .")), data = original, family = "binomial")
        ...
        ## sds
        glm_model_sds <- glm.synds(as.formula(paste0(dv, " ~ .")), data = sds_list, family = "binomial")
        ...
        output_cio$parents <- k
        output_cio$privacy <- e
        output_cio$copies <- c
