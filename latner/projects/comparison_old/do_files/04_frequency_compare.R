...
roc_univariate_round <- function(original, synthetic, var_num) {
  # create frequency tables for the original and synthetic data, on the variable
  orig_table <- as.data.frame(ftable(round(original[,var_num],0)))
  syn_table <- as.data.frame(ftable(round(synthetic[,var_num],0)))
...
df_graph <- ggplot(df_compare, aes(x = value, y = pct, fill = data)) +
  ...
  facet_nested_wrap(~type+sdg, scales = "free") +
  theme_bw() +
  scale_x_discrete(breaks = c("0","10","20","30","32","40","50","60","70","80","99","100",NA)) +
  theme(panel.grid.minor = element_blank(), 
        legend.position = "bottom",         
        # axis.title.x=element_blank(),
        legend.key.width=unit(1, "cm"),
...
#bmi
roc_univariate_round(df_ods,df_ctgan,34)
roc_univariate_round(df_ods,df_synthpop,34)
roc_univariate_round(df_ods,df_datasynthesizer,34)


df_bmi_1 <- data.frame(Measure = "ROE",
                       ctgan = roc_univariate_round(df_ods,df_ctgan,34),
                       datasynthesizer = roc_univariate_round(df_ods,df_datasynthesizer,34))

df_bmi_2 <- data.frame(Measure = "ROE",
                       ctgan = roc_univariate_round(df_ods,df_ctgan,34),
                       datasynthesizer = roc_univariate_round(df_ods,df_datasynthesizer,34),
                       synthpop = roc_univariate_round(df_ods,df_synthpop,34))
...
df_frequency <- rbind(df_bmi_2,df_edu,df_nofriend_2)
df_frequency$variable <- c("BMI","Education", "Number of friends")
...
df_graph <- ggplot(df_frequency_long, aes(x = variable, y = value)) +
  geom_bar(position = position_dodge(width = .9), stat = "identity") +
  facet_nested_wrap(~name) +
