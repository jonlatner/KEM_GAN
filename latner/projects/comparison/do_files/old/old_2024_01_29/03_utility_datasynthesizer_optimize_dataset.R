...
write.csv(df_comparison, paste0(tables,"datasynthesizer_optimize_utility_dataset.csv"), row.names=FALSE)

# Graph ----

df_comparison <- read.csv(paste0(tables,"datasynthesizer_optimize_utility_dataset.csv"))
...
df_graph <- ggplot(df_comparison, aes(x = parents, y = values)) +
  ...
  facet_nested_wrap(data ~ utility, labeller = labeller(.rows = label_both), scales = "free_y", nrow = 3) +
...
ggsave(plot = df_graph, paste0(graphs,"datasynthesizer_optimize_utility_dataset.pdf"), height = 4, width = 6)
