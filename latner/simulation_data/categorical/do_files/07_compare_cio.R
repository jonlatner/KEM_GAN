...
df_sds_datasynthesizer = read.csv(paste0(tables, "datasynthesizer/utility_cio_estimates.csv"))
df_sds_ctgan = read.csv(paste0(tables, "ctgan/utility_cio_estimates.csv"))
df_sds_synthpop = read.csv(paste0(tables, "synthpop/utility_cio_estimates.csv"))
...
df_graph <- ggplot(df_regression_data, aes(x = estimate, y = term, color = data)) +
  ...
  facet_grid(type ~ dep_var, scales="free_x", labeller = labeller(.cols = label_both)) +
...
ggsave(plot = df_graph, paste0(graphs,"graph_compare_cio_regressions.pdf"), height = 4, width = 10)
