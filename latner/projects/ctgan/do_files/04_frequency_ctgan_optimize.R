...
df_graph_data <- df_comparison %>%
  filter(variables %in% c("age","edu","income","height","weight","wkabdur","mmarital","mmarr","msepdiv")) %>%
  ...
  group_by(data,variables,epochs,dataset) %>%
  mutate(value2 = ifelse((variables == "mmarr" | variables == "msepdiv") & row_number()>1, yes = value2 +1, no = value2)) %>%
  ungroup() %>%
