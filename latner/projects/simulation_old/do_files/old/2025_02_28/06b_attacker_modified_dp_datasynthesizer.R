...
for (c in 1:10) {
  for (r in 1:16) {
    ...
    sds <- read.csv(paste0(synthetic_data,"datasynthesizer_synthetic_data.csv")) %>%
      filter(epsilon == 0.1) %>%
      filter(number == c) %>%
      select(-epsilon,-number,-copies)
...
# ggsave(plot = df_graph, paste0(graphs,"graph_attacker_dp.pdf"), height = 5, width = 10)
# ggsave(plot = df_graph, paste0(graphs,"graph_attacker_dp_v2.pdf"), height = 5, width = 5)
