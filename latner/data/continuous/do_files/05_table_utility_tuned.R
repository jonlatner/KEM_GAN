...
for (u in utility) {
  for (s in synthetic) {
    ...
    df <- df %>%
      filter(epochs == 900 & discriminator == 10 & frequency == "False" & batch == 500 & copies == 10) %>%
...
for (u in utility) {
  for (s in synthetic) {
    df <- read.csv(paste0(tables,s,"/utility_",u,".csv"))
    df <- df %>%
      filter(privacy == 0.1 & parents == 2 & copies == 10) %>%
...
for (u in utility) {
  for (s in synthetic) {
    df <- read.csv(paste0(tables,s,"/utility_",u,".csv"))
    df <- df %>%
      filter(cp == 0.00000001 & minbucket == 5 & copies == 10) %>%
...
print(latex_table, 
      ...
      file = paste0(tables,"table_utility_tuned.tex"),
)
