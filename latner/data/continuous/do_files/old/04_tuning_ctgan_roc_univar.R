...
df_graph <- ggplot(data_for_clustering, aes(y = roc, x = n)) +
  geom_point() +
  theme_bw() +
...
lm_model <- lm(roc ~ epochs + discriminator + batch + frequency + copies, data = data_for_clustering)
summary(lm_model)
...
for (c in copies_values) {
  lm_model <- lm(roc ~ epochs + discriminator + batch + frequency, data = subset(data_for_clustering, copies == c))
