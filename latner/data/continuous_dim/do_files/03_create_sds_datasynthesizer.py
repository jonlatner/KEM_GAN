...
df_duration = pd.DataFrame(columns=["type", "rows", "cols", "n", "duration"])
...
for r in rows:
    for c in cols:
        ...
        for j in range(m):    
            ...
            filename_sds = f"sds_datasynthesizer_rows_{r}_cols_{c}_n_{j}.csv"
            ...
            output = ("datasynthesizer",r,c,j,time_duration)
            df_output = pd.DataFrame([output], columns=["type", "rows", "cols", "n", "duration"])
