...
for (r in rows) {
  for (c in cols) {
    for (v in vals) {
      count = count + 1
      df_ods <- read.csv(paste0(original_data_categorical,"ods_rows_",r,"_cols_",c,"_vals_",v,".csv"))
      assign(paste0("sim_categorical",count),df_ods)
    }
  }
}
...
for (r in rows) {
  for (c in cols) {
    ...
    assign(paste0("sim_continuous_",count),df_ods)
  }
}
...
print.xtable(latex_table, 
             ...
             floating = FALSE,
             booktabs = TRUE, 
             file = paste0(tables,"data_dimensions.tex"))
