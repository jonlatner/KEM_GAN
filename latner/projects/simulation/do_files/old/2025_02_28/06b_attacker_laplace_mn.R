...
for (c in 1:10) {
  for (r in 1:16) {
    ...
    df_ods_frequency$combine <- factor(df_ods_frequency$combine, levels = apply(t(sapply(0:15, number2binary, 4)), 1, paste0, collapse = ""))
    ...
    for (e in epsilon) {
      ...
      laplace_noise <- LaplacesDemon::rlaplace(nrow(df_frequency_ods), location, b)
