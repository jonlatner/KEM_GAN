...
for (k in parents) {
  for (e in privacy) {
        for (m in copies) {
          df_sds <- read.csv(paste0(synthetic_data,"sds_datasynthesizer_tuning_e_",e,"_d_",d,"_b_",b,"_f_",f,"_m_",m,"_n_1.csv"))
          df_sds <- df_sds %>% 
            mutate_if(is.character, factor)
...
data_for_clustering <- df_pmse %>%
  group_by(parents, privacy, batch, frequency, copies) %>%
...
ggsave(plot = df_graph, paste0(graphs,"graph_datasynthesizer_spmse_raw.pdf"), height = 4, width = 6)

## Linear model

lm_model <- lm(pmse ~ parents + privacy + copies, data = data_for_clustering)
summary(lm_model)
...
df_graph
ggsave(plot = df_graph, paste0(graphs,"graph_datasynthesizer_spmse.pdf"), height = 4, width = 6)
...
for (c in copies_values) {
  lm_model <- lm(pmse ~ parents + privacy, data = subset(data_for_clustering, copies == c))
...
ggsave(plot = df_graph, paste0(graphs,"graph_datasynthesizer_spmse_facet.pdf"), height = 9, width = 6)

## Summary table 

write.csv(df_univ, paste0(tables,"utility_pmse.csv"), row.names=FALSE)
