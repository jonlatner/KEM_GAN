...
df_count <- rbind(df_count,df_long_ods_1)
 

#graph----

df_count$parents <- factor(df_count$parents, levels = c("Original", "0", "1", "2"))

df_graph <- ggplot(df_count, aes(x = value, y = n, fill = parents)) +
  geom_bar(position = position_dodge(width = 0.9), stat = "identity") +
  facet_wrap( ~ variables, scales = "free", labeller = labeller(.cols = label_both)) +
